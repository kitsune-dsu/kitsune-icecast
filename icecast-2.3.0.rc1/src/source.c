...
static void *source_fallback_file (void *arg) E_NOTELOCALS /**DSU data_add */
{
    char *mount = arg;
    char *type;
    char *path;
    unsigned int len;
    FILE * E_OPAQUE file = NULL; /**DSU xfgen_add */
    source_t *source = NULL;
    ice_config_t *config;
    http_parser_t *parser;

    MIGRATE_LOCAL(mount); /**DSU data_add */
    MIGRATE_LOCAL(file); /**DSU data_add */

    do
    {
        if (!kitsune_is_updating()) { /**DSU control_add 2 */
