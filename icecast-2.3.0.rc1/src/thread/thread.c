...
static avl_tree E_G(thread_type *,[opaque]) *_threadtree = NULL; /**DSU xfgen */

#ifdef DEBUG_MUTEXES
static mutex_t _threadtree_mutex = { -1, NULL, MUTEX_STATE_UNINIT, NULL, -1, 
    PTHREAD_MUTEX_INITIALIZER};
#else
static mutex_t _threadtree_mutex = { NULL }; /**DSU other */
#endif
...
static avl_tree E_G(mutex_t *,[opaque]) *_mutextree = NULL; /**DSU xfgen */
...
static mutex_t _library_mutex = { -1, NULL, MUTEX_STATE_UNINIT, NULL, -1,
    PTHREAD_MUTEX_INITIALIZER};
#else
/* EKIDEN: modified sys_mutex to be a pointer */
static mutex_t _library_mutex = { NULL }; /**DSU other */
#endif
        ...
        void *arg, int detached, int line, char *file)
{
    ...
    do {
        ...
        if (kitsune_pthread_create (&thread->sys_thread, &attr, _start_routine, start) == 0) /**DSU thread */
        {
...
static void _mutex_create(mutex_t *mutex)
{
    ...
    mutex->line = -1;
#endif
    /* EKIDEN: modified sys_mutex to be a pointer to a pthread_mutex */
    mutex->sys_mutex = malloc(sizeof(pthread_mutex_t)); /**DSU other 2 */
...
{
    /* EKIDEN: modified sys_mutex to be a pointer to a pthread_mutex */
    pthread_mutex_destroy(mutex->sys_mutex); /**DSU other 3 */
    free(mutex->sys_mutex);
...
{
    /* EKIDEN: modified sys_mutex to be a pointer to a pthread_mutex */
    cond->sys_cond = malloc(sizeof(pthread_cond_t)); /**DSU other 4 */
    pthread_cond_init(cond->sys_cond, NULL);
    cond->cond_mutex = malloc(sizeof(pthread_mutex_t));
    pthread_mutex_init(cond->cond_mutex, NULL);
}

void thread_cond_destroy(cond_t *cond)
{
    /* EKIDEN: modified sys_cond to be a pointer */
    pthread_mutex_destroy(cond->cond_mutex); /**DSU other 6 */
    free(cond->cond_mutex);
    cond->cond_mutex = NULL;
...
{
    /* EKIDEN: modified sys_cond to be a pointer */
    pthread_cond_signal(cond->sys_cond); /**DSU other */
}

void thread_cond_broadcast_c(cond_t *cond, int line, char *file)
{
    /* EKIDEN: modified sys_cond/cond_mutex to be pointers */
    pthread_cond_broadcast(cond->sys_cond); /**DSU other */
}
...
void thread_cond_timedwait_c(cond_t *cond, int millis, int line, char *file)
{
    ...
    /* EKIDEN: modified sys_cond/cond_mutex to be pointers */
    pthread_mutex_lock(cond->cond_mutex); /**DSU other 3 */
...
{
    /* EKIDEN: modified sys_rwlock to be a pointer */
    rwlock->sys_rwlock = malloc(sizeof(pthread_rwlock_t)); /**DSU other 2 */
...
{
    /* EKIDEN: modified sys_rwlock to be a pointer */
    pthread_rwlock_destroy(rwlock->sys_rwlock); /**DSU other 3 */
    free(rwlock->sys_rwlock);
    rwlock->sys_rwlock = NULL;
}

void thread_rwlock_rlock_c(rwlock_t *rwlock, int line, char *file)
{
    /* EKIDEN: modified sys_rwlock to be a pointer */
    pthread_rwlock_rdlock(rwlock->sys_rwlock); /**DSU other */
}

void thread_rwlock_wlock_c(rwlock_t *rwlock, int line, char *file)
{
    /* EKIDEN: modified sys_rwlock to be a pointer */
    pthread_rwlock_wrlock(rwlock->sys_rwlock); /**DSU other */
}

void thread_rwlock_unlock_c(rwlock_t *rwlock, int line, char *file)
{
    /* EKIDEN: modified sys_rwlock to be a pointer */
    pthread_rwlock_unlock(rwlock->sys_rwlock); /**DSU other */
}
...
static void *_start_routine(void *arg)
{
    ...
    /* Ekiden: removed this free because we need start to remain valid for later */
    /* free (start); */ /**DSU other 4 */
    if (kitsune_is_updating()) {
        transform_fptr(&start_routine, &start->start_routine, 0, NULL);
...
{
    /* EKIDEN: modified sys_mutex to be a pointer to a pthread_mutex */
    pthread_mutex_lock(mutex->sys_mutex); /**DSU other */
}

static void _mutex_unlock(mutex_t *mutex)
{
    /* EKIDEN: modified sys_mutex to be a pointer to a pthread_mutex */
    pthread_mutex_unlock(mutex->sys_mutex); /**DSU other */
}
