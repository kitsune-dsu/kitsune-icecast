 ...
 */
static void *start_relay_stream (void *arg) E_NOTELOCALS /**DSU data */
{
    relay_server *relay = arg;
    MIGRATE_LOCAL(relay); /**DSU data */
    ...
    {
        char *auth_header;
        if (!MIGRATE_LOCAL(streamsock)) { /**DSU data */
        ...
        if (!MIGRATE_LOCAL(con)) { /**DSU data */
            con = connection_create (streamsock, -1, NULL);
        }

        MIGRATE_LOCAL(parser); /**DSU data */
...
static void *_slave_thread(void *arg) E_NOTELOCALS /**DSU data */
{
    ice_config_t *config;
    unsigned int interval = 0;

    MIGRATE_LOCAL(interval); /**DSU data */

    if (!kitsune_is_updating()) /**DSU control_add */
        source_recheck_mounts();

    while (1)
    {
      kitsune_update("slave"); /**DSU updatepoint */
