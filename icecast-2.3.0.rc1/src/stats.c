...
void *stats_connection(void *arg) E_NOTELOCALS /**DSU data */
{
    ...
    MIGRATE_LOCAL(client); /**DSU data_chg */
    ...
    /* Ekiden Change */
    if (!MIGRATE_LOCAL(local_event_mutex)) { /**DSU data */
        thread_mutex_create(&local_event_mutex);
    }

    /* Ekiden Change */
    if (!MIGRATE_LOCAL(local_event_queue)) { /**DSU data */
        _atomic_get_and_register(&local_event_queue, &local_event_mutex);
    }

    while (_stats_running) {
      kitsune_update("stats_connection"); /**DSU updatepoint */
