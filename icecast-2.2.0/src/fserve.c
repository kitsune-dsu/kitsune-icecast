...
static avl_tree E_G(mime_type*,[opaque]) *mimetypes = NULL; /**DSU xfgen */
...
typedef struct mime_type_tag { /**DSU xfgen_ignore */
    char *ext;
    char *type;
} mime_type;
...
static void wait_for_fds() {
    ...
    while (run_fserv)
    {
      kitsune_update("fserv"); /**DSU updatepoint */
...
static void *fserv_thread_function(void *arg)
{
    ...
    if (!kitsune_is_updating()) /**DSU control 1 */
        INFO0("file serving thread started");

    while (run_fserv) {
      kitsune_update("fserv"); /**DSU updatepoint */

        if (!kitsune_is_updating_from("fserv_active")) /**DSU control */
            wait_for_fds();
        ...
        while (fclient)
        {
          kitsune_update("fserv_active"); /**DSU updatepoint */
